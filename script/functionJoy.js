...
function waitIn15Seconds(faucetName, mins, strCounDown, button){
    ...
    var tempInterval = setInterval(function(){
        ...
        if(count == 10){
            ...
            startCountDown(faucetName, mins, strCounDown, button);
        }
    }, 1000);
}

// To Set How many minutes to wait to claim BTC Satoshi
function startCountDown(faucetName, minutes, sCountDown, button){
    let date = new Date();
    var countDownDate = date.setMinutes(date.getMinutes() + minutes);
    countdown(faucetName, sCountDown, countDownDate, button);
}

// Setup Timer To Start Counting
function countdown(faucetName, element, countDownDate, button) {
    ...
    var interval = setInterval(function() {
        ...
        if(seconds == 0) {
            if(minute_text == '' && hour_text == '') {
                ...
                /*
                if(faucetName == 'EARNBITMOON'){
                    playMusic(el, wedd, 'Weedd.mp3');
                }

                if(faucetName == 'FREE LITECOIN'){
...
function setTimer(faucetName, duration, setUrl, setPoints, setCountDown, input){
    countLimit++;
    countPoints += setPoints; 
    //earnperHour += setPoints;
    window.open(setUrl);
    waitIn15Seconds(faucetName, duration, setCountDown, input);    
}
